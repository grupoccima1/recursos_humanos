$(function(){   
    ...
    $('#desarrolloid').on('change', function(){
            var id = $('#desarrolloid').val();
            var url = 'php/agrega_condominio.php';
            $.ajax({
                type:'POST',
                url:url,
                data:'id='+id,
                success: function(data){
                    $('#clusterid option').remove();
                    $('#clusterid').append(data);
                } 
            });
            return false;
        });
...
$(function() {
    $('#desarrolloid').change(function() {
        localStorage.setItem('dev', this.value);
    });
    
    if(localStorage.getItem('dev')){
        $('#desarrolloid').val(localStorage.getItem('dev'));
    }
});
...
const select = document.getElementById("clusterid");
